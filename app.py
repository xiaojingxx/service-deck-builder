    ...
    with st.expander("3. Service Order", expanded=True):
        ...
        if not setlist:
            ...
            for i, song in enumerate(setlist):
                song_id = song.get("song_id")
                if not song_id:
                    song_id = f"fallback_{song.get('umh_number', '')}_{song.get('title', '')}_{i}"
                ...
                if song["umh_number"]:
                    label = f'{i+1}. UMH {song["umh_number"]} {song["title"]}{section_suffix} ({len(song["slides"])})'
                else:
                    label = f'{i+1}. {song["title"]}{section_suffix} ({len(song["slides"])})'

                labels_by_song_id[song_id] = label
                song_ids_in_order.append(song_id)

            selected_song_id = st.session_state.get("selected_song_id")
            if selected_song_id not in song_ids_in_order:
                selected_song_id = song_ids_in_order[0] if song_ids_in_order else None
                st.session_state["selected_song_id"] = selected_song_id

            selected_index_for_widget = song_ids_in_order.index(selected_song_id) if selected_song_id in song_ids_in_order else 0
            widget_key = f"selected_song_id_selectbox_v{st.session_state['song_selectbox_version']}"

            previous_selected_index = next(
                (i for i, song in enumerate(setlist) if song.get("song_id") == st.session_state.get("selected_song_id")),
                0,
            )

            selected_song_id = st.selectbox(
                "Selected song",
                options=song_ids_in_order,
                index=selected_index_for_widget,
                format_func=lambda sid: labels_by_song_id[sid],
                key=widget_key,
            )
        ...
        if st.button("Import Songs from DOCX", use_container_width=True):
            if service_docx_file is None:
                ...
                try:
                    ...
                    st.session_state["selected_song_id"] = st.session_state["setlist"][0].get("song_id") if st.session_state["setlist"] else None
                    st.session_state["song_selectbox_version"] += 1
