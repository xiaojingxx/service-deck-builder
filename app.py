...
def cursor_line_to_slide(raw_text: str, cursor_line: int, lines_per_slide: int) -> int:
    """
    Maps editor cursor line to the slide number in auto-split mode.
    For manual mode, blank-line blocks define slides.
    """
    ...
    if not st.session_state["auto_split_by_lines"]:
        ...
        for i, raw in enumerate(lines):
            ...
            current_blank = raw.strip() == ""
            if current_blank and not previous_blank:
                # next non-blank block begins a new slide
                pass
            elif not current_blank and previous_blank and i != 0:
                slide_num += 1

            previous_blank = current_blank

        return max(1, slide_num)
    ...
    for i, raw in enumerate(lines):
        ...
        verse_line_count += 1
        if verse_line_count > lines_per_slide:
            slide_num += 1
            verse_line_count = 1
...
# =========================
def build_preview_signature(song_item, template_name, auto_split, lines_per_slide):
    return (
...
with editor_col:
    ...
    editor_state = live_editor(
        text=st.session_state.get("editor_text", ""),
        height=760,
        key="lyrics_live_editor",
    )

    editor_text = editor_state.get("text", "")
    cursor_line = editor_state.get("cursorLine", 0)
    ...
    if slide_count > 0:
        focused_slide = cursor_line_to_slide(
            editor_text,
            cursor_line,
            st.session_state["lines_per_slide"],
        )
        focused_slide = min(max(focused_slide, 1), slide_count)
        st.session_state["focused_preview_slide"] = focused_slide
        st.caption(f"{slide_count} slide(s) detected. Cursor is on slide {focused_slide}.")
    else:
        st.session_state["focused_preview_slide"] = 1
        st.caption("No slides detected yet.")
...
if not current_slides:
    ...
    if st.session_state.get("last_preview_signature") != preview_signature:
        try:
